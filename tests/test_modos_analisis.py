...
def test_reporte_modo_excel_organigrama_incluye_columnas_excel_requeridas():
    encabezados = _encabezados(ModoAnalisis.EXCEL_ORGANIGRAMA)
    columnas_excel = [e for e in encabezados if str(e).startswith("Excel -")]
    assert columnas_excel == [
        "Excel - Original",
        "Excel - Asimilado",
        "Excel - Nivel",
        "Excel - Estado de nivel",
    ]
    assert not any(str(e).startswith("Word -") for e in encabezados)
